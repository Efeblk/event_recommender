...
5. **AI evaluation:** provision the selected provider key through secure local/server settings. The existing chat provider path needs live Turkish quality and latency tests. Jev has a separate opt-in evaluation harness; inspect Turkish negation, follow-ups, unsupported preferences and real-catalog recall before connecting it to the app. Record actual usage and establish limits. No live model evaluation has been claimed from mocked tests.
