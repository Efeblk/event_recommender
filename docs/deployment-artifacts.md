...
The compiled directory and provenance directory are uploaded as one workflow artifact, including hidden Vite metadata such as `.vite/manifest.json` and asset metadata directories. Preparation immediately downloads that artifact into an isolated runner directory and verifies every path and digest again. The protected `deploy` job cannot become approval-eligible until this round trip passes. It later downloads the same prepared artifact and verifies the complete compiled manifest before generating the selected environment's Wrangler configuration. Target-specific Worker, D1, and R2 bindings therefore remain outside the immutable compiled manifest while staging and production consume identical preparation rules. Manifest comparison uses exact, case-sensitive paths and hashes rather than locale-sensitive object order.
