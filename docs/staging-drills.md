...
7. Generate a cryptographically random object name under `drills/checkpoint-restore/<revision>/` in the staging R2 bucket. Write the saved checkpoint bytes to only that isolated object and verify its hash and schema/count semantics; overwrite only that object with a deterministic corrupt fixture and verify the corruption; restore the saved bytes and verify them again. Delete the generated object only after every phase succeeds. A failure preserves the evidence and isolated object key for investigation rather than attempting cleanup.
8. Preserve `source.sql`, `checkpoint.json`, `r2-restore.json`, and `result.json`, including SHA-256 hashes and the redacted command sequence. No secret value is written.

If any step fails, preserve the artifact directory, isolated R2 drill object, and disposable database for investigation. Delete them manually only after reviewing their exact names in the evidence and Cloudflare dashboard or Wrangler listings. D1 cleanup is intentionally outside the tool. The successful R2 path deletes only its generated drill key. This proves byte-for-byte restoration of an actual checkpoint in an isolated remote object; it does not modify, corrupt, replace, or exercise failover of the live canonical checkpoint key.
